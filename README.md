...
The Kubernetes integration works locally against [Rancher Desktop](https://rancherdesktop.io/)
(k3s). After installing:

```bash
# Verify the context is active
kubectl config current-context   # should print: rancher-desktop
kubectl cluster-info             # should show the local API server

# The API auto-detects ~/.kube/config — no extra config needed.
